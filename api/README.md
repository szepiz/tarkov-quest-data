...
| `objectiveTextById` | the bridge between those two, on **223 quests**: the game's wording keyed by the objective id it belongs to, so anything rendering the structured list can show it. Matched by CONTENT, never by index — every objective has to claim one line with a clear margin over the runner-up or the whole quest is left alone, which is why 74 quests have none (*Pyramid Scheme*'s fifteen near-identical ATM steps, *Bad Habit*'s six objectives against the game's two). What a step asks you to DO has to agree as well as its wording: *No Swiping*'s "Eliminate any target at the smuggler bases" shares more words with "Locate the smugglers' base" than with the eliminate objective, and pairing those would put the sentence on the wrong pin |
