...
| `objectiveTextById` | the bridge between those two, on **234 quests**: the game's wording keyed by the objective id it belongs to, so anything rendering the structured list can show it. Matched by CONTENT, never by index — every objective has to claim one line with a clear margin over the runner-up or the whole quest is left alone, which is why 66 quests have none (*Pyramid Scheme*'s fifteen near-identical ATM steps, *Bad Habit*'s six objectives against the game's two). What a step asks you to DO has to agree as well as its wording: *No Swiping*'s "Eliminate any target at the smuggler bases" shares more words with "Locate the smugglers' base" than with the eliminate objective, and pairing those would put the sentence on the wrong pin. **One line on each side is exempt**, because a margin is a margin over a runner-up and there isn't one: 93 pair that way, and they are the cases where the wording can differ completely and the pairing still be forced — *Hunting Trip* publishes "kill Shturman with a headshot from over 40m using an M700" against the game's "twenty bosses with any bolt-action" |
