...
| **316 quests read off the game screen** | Names, objectives, trader, loyalty tab and status, with the date each was seen |
| **90 quests the game files under Essential** | Its trader list is tabbed by loyalty level with one more tab beside them, and no source publishes which tab anything is in. `traderTab` is that tab: LL1 64, LL2 74, LL3 61, LL4 27, Essential 90. The card carries it too — the badge in its top-right corner is a crown at LL4, three bars at LL3 and lines for Essential |
| **Objectives worded as the game words them** | 239 quests carry the wording against the objective ids a tracker ticks with. Not a formatting preference: the wiki asks for 2 ComTac II headsets where the game asks for one, sends *Job for a Patriot* to three maps it no longer uses, and gives *No Swiping* no map at all |
...
else: 316 quests read off my own game screen and written down, which is what the
...
| **observed/** | 316 quests read off the in-game screen | mine, CC0. Not a fifth opinion, it's the thing the other four are describing |
...
2026-08-10 onward. 316 records: 194 completed, 117 active, 4 failed, 1 locked.
...
Graded against those 316 records: **wiki 716 of 750 (95%)**, tarkov.dev 920 of
